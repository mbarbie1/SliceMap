...
public class LibRoi {
    ...
    /**
     * Interpolates multiple ROIs by taking a (sum) z-projection of the stack of
     * ROIs
     *
     * @param rois
     * @param sizeX
    ...
    /**
     * When having only the imagej zip-files of the ROI's, convert them to a
     * csv-file
     *
     * @param roiFile
     * @param outputFile
...
//		ts.run(imp);
     ...
     * @return
     */
    public static ImagePlus getInterpolationOverlayImage(ArrayList< LinkedHashMap< String, Roi>> rois, LinkedHashMap< String, Roi> interpRois, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
     ...
     * @return
     */
    public static ImagePlus getOverlayImage(LinkedHashMap< String, Roi> rois, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
     ...
     * @return
     */
    public static ImagePlus getOverlayImageRGB(LinkedHashMap< String, Roi> rois, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
     ...
     * @return
     */
    public static ImagePlus getConfidenceBandOverlayImage(LinkedHashMap< String, LinkedHashMap< String, Roi>> ciMap, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
     ...
     * @return
     */
    public static ImagePlus getConfidenceOverlayImage(LinkedHashMap< String, LinkedHashMap< String, Roi>> ciMap, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
     ...
     * @param rois
     * @param imp
     * @return public static ImagePlus getConfidenceOverlayImage( LinkedHashMap< String, Roi
     * > roiMap, LinkedHashMap< String, ImagePlus > ciMaskMap, ImagePlus imp ) {
     *
     * ImagePlus impRois = new ImagePlus("Image with rois",
     * imp.getProcessor().duplicate().convertToRGB()); Overlay overlay = new
     * Overlay(); for ( String key : ciMaskMap.keySet() ) {
     *
     * ImagePlus mask = ciMaskMap.get(key); Roi roi = roiMap.get(key);
     * roi.setStrokeWidth(3); innerRoi.setStrokeWidth(1);
     * outerRoi.setStrokeWidth(1); String name = key; if (name != null &&
     * key.startsWith("EVT_Regions_")) { name = key.substring(
     * key.lastIndexOf("_") + 1, key.length()); } Color color =
     * roiColor().get(name); if (color != null) { } else if (
     * roi.getStrokeColor() != null) { color = roi.getStrokeColor(); } else {
     * color = Color.GRAY; } roi.setStrokeColor(color); overlay.add( roi, name);
     * overlay.add( innerRoi, name+"_inner"); overlay.add( outerRoi,
     * name+"_outer"); }
     *
     * overlay.setLabelFont(new Font("fontName", Font.PLAIN, (int)
     * Math.round(impRois.getWidth() / 50.0))); impRois.setOverlay(overlay);
     * impRois.setHideOverlay(false);
     *
     * return impRois; }
     */
     ...
     * @return
     */
    public static ImagePlus getOverlayImage(Roi[] rois, ImagePlus imp, LinkedHashMap<String, Color> roiColor) {
        ImagePlus impRois = new ImagePlus("Image with rois", imp.getProcessor().duplicate().convertToRGB());
        Overlay overlay = new Overlay();
...
//			overlay.setStrokeColor( color ); 
    ...
    public static LinkedHashMap<String, Color> getRoiColors(File roiDefinitionsFile) {
